...
- "new_option" — a genuinely new action not covered by an existing option
  ...
  **Decide whether this genuinely replaces the existing options or sits
  alongside them.** These are different things, and confusing them was a
  real, confirmed complaint: delegating a task changes who owns it — the
  existing "you personally respond" options usually stop making sense once
  someone else is handling it, and should be superseded, not left sitting
  next to something new. A new option that's a genuinely separate,
  additional path (asking a clarifying question of the same person, say)
  should NOT supersede anything. When in doubt, ask yourself: if this new
  option happens, do the old ones still describe real choices, or were they
  only ever about a version of the situation that's now gone? List every
  option number that no longer applies in "supersedes" — usually all of
  them when this is a delegation reframing the whole ticket, sometimes none
  when this is genuinely additive.
